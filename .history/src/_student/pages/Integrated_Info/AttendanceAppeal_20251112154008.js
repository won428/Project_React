...
function AttendanceAppeal() {
    ...
    const lectureDateFromCheck = state?.lectureDate || null;
    ...
    const [selectedDates, setSelectedDates] = useState(
        lectureDateFromCheck ? [new Date(lectureDateFromCheck)] : []
    );
    ...
    const handleSubmit = (e) => {
        ...
        if (!selectedDates.length) return alert("날짜를 최소 1개 선택하세요.");
        ...
        selectedDates.forEach((date, idx) => {
            formData.append(`attendanceDates[${idx}]`, date.toISOString().slice(0, 10));
        });
    ...
    return (
        <Container style={{ maxWidth: 720, marginTop: 24, marginBottom: 24 }}>
            ...
            <Form onSubmit={handleSubmit}>
                ...
                {/* ✅ 날짜 선택 */}
                <Form.Group className="mb-3">
                    <Form.Label>출결 날짜 선택</Form.Label>
                    <DatePicker
                        selected={null}
                        onChange={date => {
                    ...
                    <div style={{ marginTop: 8 }}>
                        선택된 날짜: {selectedDates.map(d => d.toISOString().slice(0, 10)).join(", ")}
                    </div>
                </Form.Group>

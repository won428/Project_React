...
function App() {
        ...
        axios.get(`${API_BASE_URL}/api/student/record/${recordId}`)
            .then(res => {
                ...
                setForm({
                    ...
                    startDate: data.targetStartDate ? data.targetStartDate.slice(0, 10) : today,
    ...
    const submitForm = () => {
        const body = {
            ...
            startDate: showCalendar ? form.startDate : null,
            endDate: showCalendar && (form.studentStatus === 'MEDICAL_LEAVE' || form.studentStatus === 'MILITARY_LEAVE' || form.studentStatus === 'REINSTATED')
                ? form.endDate
                : null,
            status: 'PENDING'
        };
            ...
            <Form onSubmit={onSubmit}>
                ...
                {showCalendar && (
                    <Row className="mb-3">
                        ...
                        {(form.studentStatus === 'MEDICAL_LEAVE' || form.studentStatus === 'MILITARY_LEAVE' || form.studentStatus === 'REINSTATED') && (
                            <Col md={6}>

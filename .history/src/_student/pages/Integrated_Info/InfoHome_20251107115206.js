...
function App() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_BASE_URL}/api/student/record`, { params: { userId: user?.id } })
      .then(res => {
  ...
  const handleUploadImage = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/api/student/status/upload_image`, formData, {
  ...
  return (
    <Container>
          ...
          <tr>
            <td style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'center', width: '105px', height: '135px' }}>
              {previewURL ? (
                ...
                <img src={statusRecords.studentImage} alt="증명사진"
                  style={{ width: '105px', height: '135px', objectFit: 'cover', display: 'block' }} />
              ) : (
                '-'
              )}
      ...
      <Form.Group className="mb-3" style={{ width: '250px' }}>
        <Form.Label>증명사진 변경</Form.Label>
        <Form.Control type="file" accept="image/*" onChange={handleFileChange} />
      </Form.Group>

      <Button variant="success" size="sm" onClick={handleUploadImage} style={{ marginBottom: '2rem' }}>
        증명사진 저장
      </Button>

...
function App() {
  ...
  useEffect(() => {
    ...
    console.log("server statusRecords:", res.data.statusRecords);
      ...
      .then(res => {
        if (res.data.type === 'STUDENT') {
          ...
          if (updatedStatusRecords.studentImage) {
            updatedStatusRecords.studentImage = `${API_BASE_URL}/images/${updatedStatusRecords.studentImage}`;
          }

          setStatusRecords(updatedStatusRecords);
          setPreviewURL(updatedStatusRecords.studentImage); // previewURL도 절대 URL로 세팅
          setError(null);
        } else {
          setStudentInfo(null);
  ...
  const handleFileInputChange = async (e) => {
    ...
    try {
      ...
      // 서버에서 반환한 URL로 preview 업데이트
      setPreviewURL(`${API_BASE_URL}${response.data}`);

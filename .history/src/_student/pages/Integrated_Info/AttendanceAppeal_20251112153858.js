...
const ATTENDANCE_DETAILS = [
    ...
    { value: 'FAMILY_REASON', label: '집안 사정' },   // 새로 추가
...
function AttendanceAppeal() {
    ...
    const { state } = useLocation(); // 모달에서 navigate로 전달된 값
    const lectureDateFromCheck = state?.lectureDate || '';
    const attendanceFromModal = state?.attendanceType; // "BE_LATE" or "ABSENT" 등
    ...
    const [appealForm, setAppealForm] = useState({
        ...
        sendingId: userId || '',
        receiverId: '',
        appealType: 'ATTENDANCE', // 항상 출결
        attendanceType: attendanceFromModal || 'BE_LATE', // 모달 값으로 초기화
        attendanceDetail: '',     // 세부 사유 선택
        lectureDate: lectureDateFromCheck || '', // 체크한 강의일
        title: '',
        content: '',
    ...
    // 모달에서 넘어온 출결 상태를 appealForm에 반영
    useEffect(() => {

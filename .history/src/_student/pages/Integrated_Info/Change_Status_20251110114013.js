...
function App() {
    ...
    const { user } = useAuth(); // 내부에서 토큰 등 관리된다고 가정
    ...
    const [studentApply, setStudentApply] = useState(null); // 학생 신청 데이터
    ...
    axios.get(`${API_BASE_URL}/api/student/record/${recordId}`)
        .then(res => {
            setStudentApply(res.data); // 신청 폼 데이터를 상태에 저장
        })
        .catch(err => console.error('신청 데이터 로드 실패:', err));
}, [recordId]);

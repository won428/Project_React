...
function CreditAppealList() {
    ...
    const handleAdd = () => navigate('/CreditAppeal');

    const handleView = (appealId) => navigate(`/CreditAppeal?appealId=${appealId}&readonly=true`);

    const handleEdit = (appealId) => navigate(`/CreditAppeal?appealId=${appealId}`);
        ...
        <Container fluid className="py-4">
            <Row className="align-items-center mb-3">
                ...
                <Col md={6} className="text-end">
                    <Button variant="primary" onClick={handleAdd}>이의신청하기</Button>
                </Col>
            </Row>
                        ...
                        {appealList.map(appeal => (
                            <tr key={appeal.appealId}>
                                <td
                                    ...
                                    onClick={() => handleView(appeal.appealId)}
                                >
                                    {appeal.lectureName}
                                </td>
                                <td>{appeal.content}</td>
                                <td>{appeal.appliedDate}</td>
                                <td>{statusName[appeal.status]}</td>
                                <td>
                                    <Button
                                        size="sm"
                                        variant="primary"
                                        className="me-2"
                                        onClick={() => handleEdit(appeal.appealId)}
                                    >
                                        수정
                                    </Button>
                                </td>

import React, { useEffect, useState } from 'react';
import { Container, Row, Col, Form, Button } from 'react-bootstrap';
import axios from 'axios';
import { useAuth } from '../../../public/context/UserContext';
import { API_BASE_URL } from '../../../public/config/config';
import { useNavigate } from 'react-router-dom';

const APPEAL_TYPES = [
    { value: 'BE_LATE', label: '지각' },
    { value: 'ABSENT', label: '결석' }
];

const ATTENDANCE_DETAILS = [
    { value: 'MEDICAL_PROBLEM', label: '병원 방문' },
    { value: 'MILITARY_PROBLEM', label: '예비군 훈련' },
]

function AttendanceAppeal() {
    const { user } = useAuth();
    const navigate = useNavigate();
    const userId = user?.id;

    // 수강 강의 목록
    const [lectures, setLectures] = useState([]);
    const [error, setError] = useState(null);

    // 선택된 강의 및 교수
    const [selectedLecture, setSelectedLecture] = useState(null);
    const [professorId, setProfessorId] = useState('');
    const [professorName, setProfessorName] = useState('');

    // 폼 데이터
    const [appealForm, setAppealForm] = useState({
        lectureId: Number(''),
        sendingId: userId || '',
        receiverId: professorId || '',    // 담당 교수
        appealType: 'ATTENDANCE',
        attendanceDetail: '',     // 프론트 선택 사항
        title: '',
        content: '',
        appealDate: new Date().toISOString().slice(0, 10),
        status: 'PENDING'
    });

    // 첨부파일
    const [file, setFile] = useState(null);

    // 학생 수강 강의 조회
    useEffect(() => {
        axios.get(`${API_BASE_URL}/lecture/enrollments`, { params: { userId } })
            .then(res => {
                setLectures(res.data);
            })
            .catch(err => {
                console.error(err);
                setError('수강 강의를 불러오는 데 실패했습니다.');
            });
    }, [userId]);

    // 강의 선택 시 담당 교수 정보 가져오기
    useEffect(() => {
        if (!selectedLecture) return;

        axios.get(`${API_BASE_URL}/lecture/${selectedLecture}`)
            .then(res => {
                setProfessorId(res.data.userId);   // 교수 ID
                setProfessorName(res.data.userName); // 교수 이름
                setAppealForm(prev => ({
                    ...prev,
                    lectureId: selectedLecture,
                    receiverId: res.data.userId
                }));
            })
            .catch(err => console.error(err));
    }, [selectedLecture]);

    const handleChange = (e) => {
        const { name, value } = e.target;
        setAppealForm(prev => ({ ...prev, [name]: value }));
    };

    const handleFileChange = (e) => {
        setFile(e.target.files[0]);
    };

    const handleSubmit = (e) => {
        e.preventDefault();

        if (!appealForm.lectureId) {
            alert('강의를 선택해주세요.');
            return;
        }

        const formData = new FormData();
        // 서버 저장용 content에 선택사항 포함
        const contentWithDetail = `[${appealForm.attendanceDetail}] ${appealForm.content}`;

        formData.append('lectureId', appealForm.lectureId);
        formData.append('sendingId', appealForm.sendingId);
        formData.append('receiverId', appealForm.receiverId);
        formData.append('appealType', appealForm.appealType); // ATTENDANCE 고정
        formData.append('title', appealForm.title);

        formData.append('content', `[${appealForm.attendanceDetail}] ${appealForm.content}`);
        


        if (file) {
            formData.append('file', file);
        }

        axios.post(`${API_BASE_URL}/lecture/attendanceAppeal`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
        })
            .then(() => {
                alert('출결 이의제기 신청이 완료되었습니다.');
                navigate('/AttendanceAppealList');
            })
            .catch(err => {
                console.error(err);
                alert('신청 제출 중 오류가 발생했습니다.');
            });
    };

    return (
        <Container style={{ maxWidth: 720, marginTop: 24, marginBottom: 24 }}>
            <h3 style={{ marginBottom: 16 }}>출결 이의제기 신청</h3>

            <Form onSubmit={handleSubmit} style={{ marginTop: 24 }}>
                {/* 1. 강의 선택 */}
                <Form.Group className="mb-3">
                    <Form.Label>강의 선택</Form.Label>
                    <Form.Select
                        value={selectedLecture || ''}
                        onChange={e => setSelectedLecture(e.target.value)}
                        required
                    >
                        <option value="">강의를 선택하세요</option>
                        {lectures.map(lec => (
                            <option key={lec.lectureId} value={lec.lectureId}>
                                {lec.lecName}
                            </option>
                        ))}
                    </Form.Select>
                </Form.Group>

                {/* 2. 담당 교수 */}
                <Form.Group className="mb-3">
                    <Form.Label>담당 교수</Form.Label>
                    <Form.Control type="text" readOnly value={professorName} />
                </Form.Group>

                {/* 3. 이의제기 유형 선택 */}
                <Form.Group className="mb-3">
                    <Form.Label>이의제기 유형</Form.Label>
                    <Form.Select
                        name="appealType"
                        value={appealForm.appealType}
                        onChange={handleChange}
                        required
                    >
                        <option value="">선택하세요</option>
                        {APPEAL_TYPES.map(type => (
                            <option key={type.value} value={type.value}>
                                {type.label}
                            </option>
                        ))}
                    </Form.Select>
                </Form.Group>

                {/* 4. 출결 세부 선택 (appealType이 출결 관련일 때만 활성화) */}
                {appealForm.appealType === 'BE_LATE' || appealForm.appealType === 'ABSENT' ? (
                    <Form.Group className="mb-3">
                        <Form.Label>출결 세부 선택</Form.Label>
                        <Form.Select
                            name="attendanceDetail"
                            value={appealForm.attendanceDetail || ''}
                            onChange={handleChange}
                            required
                        >
                            <option value="">선택하세요</option>
                            {ATTENDANCE_DETAILS.map(o => (
                                <option key={o.value} value={o.value}>
                                    {o.label}
                                </option>
                            ))}
                        </Form.Select>
                    </Form.Group>
                ) : null}

                {/* 5. 제목 */}
                <Form.Group className="mb-3">
                    <Form.Label>제목</Form.Label>
                    <Form.Control
                        name="title"
                        value={appealForm.title}
                        onChange={handleChange}
                        placeholder="제목을 입력하세요"
                        required
                    />
                </Form.Group>

                {/* 6. 내용 */}
                <Form.Group className="mb-3">
                    <Form.Label>내용</Form.Label>
                    <Form.Control
                        as="textarea"
                        rows={5}
                        name="content"
                        value={appealForm.content}
                        onChange={handleChange}
                        placeholder="이의제기 내용을 입력하세요"
                        required
                    />
                </Form.Group>

                {/* 7. 첨부파일 */}
                <Form.Group className="mb-3">
                    <Form.Label>증빙 파일 첨부 (예: 진료내역, 예비군 필증)</Form.Label>
                    <Form.Control type="file" onChange={handleFileChange} />
                </Form.Group>

                {/* 8. 제출/취소 버튼 */}
                <Button type="submit" variant="primary" style={{ marginRight: 8 }}>
                    신청 제출
                </Button>
                <Button variant="secondary" onClick={() => navigate(-1)}>
                    취소
                </Button>
            </Form>

        </Container>
    );
}

export default AttendanceAppeal;

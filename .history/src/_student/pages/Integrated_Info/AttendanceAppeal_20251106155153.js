        ...
        axios.get(`${API_BASE_URL}/lecture/${lectureId}`)
            .then(res => {
                ...
                setAppealForm(prev => ({
                    ...prev,
                    lectureId: Number(lectureId),
                    receiverId: res.data.userId // 교수 ID
                }));
            })
            .catch(err => console.error(err));
    }, [lectureId]);
        ...
        <Container style={{ maxWidth: 720, marginTop: 24, marginBottom: 24 }}>
            ...
            {enrollments.length > 0 ? (
                enrollments.map((enroll) => (
                    <div key={enroll.Id} style={{ marginBottom: 16, padding: 12, border: '1px solid #ddd', borderRadius: 8 }}>
                        <Form.Group className="mb-2">
                            <Form.Label>강의</Form.Label>
                            {/* lectureName 변수 사용 */}
                            <Form.Control type="text" readOnly value={lectureName || enroll.lectureName} />
                        </Form.Group>
                        <Form.Group className="mb-2">
                            <Form.Label>담당 교수</Form.Label>
                            {/* professorName 변수 사용 */}
                            <Form.Control type="text" readOnly value={professorName || enroll.professorName} />
                        </Form.Group>
                    </div>
                ))
            ) : (

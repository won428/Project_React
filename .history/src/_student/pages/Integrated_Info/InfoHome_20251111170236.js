...
function App() {
  ...
  const [previewURL, setPreviewURL] = useState(null); // 브라우저 미리보기용
      ...
      .then(res => {
        if (res.data.type === 'STUDENT') {
          ...
          if (res.data.statusRecords.studentImage) {
            setPreviewURL(`${API_BASE_URL}/images/${res.data.statusRecords.studentImage}`);
          }
          setError(null);
        } else {
          setStudentInfo(null);
  ...
  const handleFileInputChange = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        alert("이미지 업로드 성공");
        // 서버에서 실제 저장된 이미지 경로 반환
        setPreviewURL(`${API_BASE_URL}/images/${response.data.studentImage}`);
      }
    } catch (err) {
      console.error(err);
    ...
    <Container>
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', tableLayout: 'fixed' }}>
        ...
        <tbody>
          ...
          <tr><th style={{ border: '1px solid #ddd', padding: '8px' }}>사용자 유형</th><td style={{ border: '1px solid #ddd' }}>{userTypeMap[studentInfo.type] || studentInfo.type}</td></tr>
        </tbody>
      </table>
      ...
      <table style={{ borderCollapse: 'collapse', width: '100%', marginTop: '1rem', tableLayout: 'fixed' }}>
        <tbody>
          ...
          <tr><th style={{ border: '1px solid #ddd', padding: '8px' }}>입학일</th><td style={{ border: '1px solid #ddd' }}>{statusRecords.admissionDate}</td></tr>

...
function App() {
    ...
    const openModal = (appeal) => {
        const aScore = appeal.ascore || 0;
        const asScore = appeal.asScore || 0;
        const tScore = appeal.tscore || 0;
        const ftScore = appeal.ftScore || 0;
        const totalScore = aScore + asScore + tScore + ftScore;
        const lectureGrade = parseFloat((totalScore / 4).toFixed(2));
    ...
    const handleScoreChange = (e) => {
        ...
        if (value === "" || /^\d+$/.test(value)) {
            ...
            const grade = parseFloat((total / 4).toFixed(2));
            setUpdatedScores({ ...newScores, totalScore: total, lectureGrade: grade });
        }
    };

        ...
        <Container style={{ marginTop: 24 }}>
                        ...
                        {filteredAppeals.length
                            ? filteredAppeals.map(a => (
                                <tr key={a.appealId}>
                                    ...
                                    <td>{a.studentCode}</td>
                                    <td>
                                        <Button variant="link" className="p-0" onClick={() => openModal(a)}>
                                            {a.title}
                                        </Button>
                                    </td>
                                    <td>{a.appealDate}</td>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)} centered>
                <Modal.Header closeButton>
                    <Modal.Title>{selectedAppeal?.appealType === "ATTENDANCE" ? "출결 이의제기 조회" : "학생 성적표 조회"}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    {selectedAppeal && (
                        <Form>
                            {["GRADE", "ASSIGNMENT", "MIDTERMEXAM", "FINALEXAM"].includes(selectedAppeal.appealType) ? (
                                    ...
                                    {["aScore", "asScore", "tScore", "ftScore"].map(f => (
                                        <Form.Group className="mb-2" key={f}>
                                            ...
                                            <Form.Control
                                                type="number"
                                                name={f}
                                                value={updatedScores[f] === 0 ? "" : updatedScores[f]}
                                ...
                                <>
                                    ...
                                    <Form.Group className="mb-2">
                                        ...
                                        <Form.Select value={selectedAppeal.status} disabled>

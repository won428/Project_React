    ...
    const openModal = async (appeal, mode) => {
        if (appeal.appealType === "ATTENDANCE") {
            try {
                ...
                // 여기서 attachments를 미리 가져올 필요 없이 빈 배열로 둬도 됨
            ...
            // attachments를 미리 가져오지 않고도 다운로드 버튼에서 API 호출
            ...
            {modalMode === "gradeView" && selectedAppeal && (
                <Modal show onHide={() => setModalMode("")} centered>
                    ...
                    <Modal.Body>
                        ...
                        {selectedAppeal && selectedAppeal.attachments?.length > 0 && (
                            <Form.Group className="mb-3">
                                ...
                                <ul style={{ paddingLeft: 0, listStyle: "none" }}>
                                    {selectedAppeal.attachments.map((file, i) => (
                                        <li key={i} className="mb-1 d-flex align-items-center justify-content-between">
                                            ...
                                            <Button
                                                size="sm"
                                                onClick={() => downloadFile(file.storedKey || file.id, file.name)}
                                            >
                                                다운로드
                                            </Button>
                                        </li>

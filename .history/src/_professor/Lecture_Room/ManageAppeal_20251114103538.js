...
function ManageAppeal() {
    ...
    const downloadFile = async (storedKey, fileName) => {
        try {
            const res = await axios.get(`${API_BASE_URL}/notice/files/download/${storedKey}`, {
                            ...
                            <Form.Group className="mb-3">
                                ...
                                <ul style={{ paddingLeft: 0, listStyle: "none" }}>
                                    {selectedAppeal.attachments.map((file, i) => (
                                        <li key={i}>
                                            <Form.Control type="text" value={file.name} disabled />
                                        </li>
                                    ))}
                                </ul>
                            </Form.Group>
                                ...
                                <ul style={{ paddingLeft: 0, listStyle: "none" }}>
                                    {selectedAppeal.attachments.map((file, i) => (
                                        <li key={i} className="mb-1 d-flex align-items-center">
                                            <Form.Control type="text" value={file.name} disabled style={{ flex: 1, marginRight: "8px" }} />

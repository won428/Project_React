...
import AttendanceAppeal from "../_student/pages/Integrated_Info/AttendanceAppeal"; // ✅ 경로 맞춰서 추가
        ...
        <Container style={{ marginTop: 24 }}>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)} centered>
                ...
                <Modal.Body>
                    {selectedAppeal && (
                        <>
                            {modalMode === "view" && selectedAppeal && selectedAppeal.appealType === "ATTENDANCE" && (
                                <AttendanceAppeal readOnly={true} appealData={{
                                    lectureId: selectedAppeal.lectureId,
                                    lectureName: selectedAppeal.lectureName,   // 서버에서 가져오도록 처리
                                    professorId: selectedAppeal.receiverId,
                                    professorName: selectedAppeal.professorName, // 서버에서 가져오도록 처리
                                    sendingId: selectedAppeal.sendingId,
                            ...
                            {selectedAppeal.appealType !== "ATTENDANCE" && (
                                <Form.Group className="mb-2">
                                    <Form.Label>이의제기 점수 항목</Form.Label>
                                    <Form.Control
                                        type="text"
                                        value={

...
function ManageAppeal() {
    ...
    // 페이지 건드리지 않고 아래 유틸함수 추가
    const downloadFile = (fileId, fileName) => {
        // 단순히 a.href로 다운로드
        const link = document.createElement('a');
        link.href = `${API_BASE_URL}/api/appeals/files/download/${fileId}`;
        link.download = fileName || 'file'; // 다운로드될 파일명 지정
    ...
    const openModal = async (appeal, mode) => {
        if (appeal.appealType === "ATTENDANCE") {
            try {
                ...
                let attachments = [];
                try {
                    const fileRes = await axios.get(`${API_BASE_URL}/api/appeals/${appeal.appealId}/attachments`);
                    attachments = fileRes.data || [];
                    console.log("attachments:", attachments); // 확인용
                } catch (err) {
                    console.warn("첨부파일 없음 또는 로드 실패", err);
                }

                // 2️⃣ ATTENDANCE / GRADE 구분
                if (appeal.appealType === "ATTENDANCE") {

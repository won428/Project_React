...
function ManageAppeal() {
    ...
    const openModal = (appeal, mode) => {
        // 출결인 경우: attendanceDate 설정
        if (appeal.appealType === "ATTENDANCE") {
            // 만약 appeal 객체에 이미 attendanceDate가 있다면 그대로 사용
            // 배열 형태의 출결 기록이 있다면 첫 차시 날짜 사용
            const attendanceDate =
                appeal.attendanceDate ||
                appeal.attendanceRecords?.[0]?.attendanceDate ||
                "";

            setSelectedAppeal({ ...appeal, attendanceDate });

...
function ManageAppeal() {
    ...
    // 🔹 모달 열기: 단건 API로 상세 조회
    const openModal = async (appeal, mode = "view") => {
        try {
            const res = await axios.get(`${API_BASE_URL}/api/appeals/${appeal.appealId}`);
            const detail = res.data;

            const { totalScore, lectureGrade } = calculateTotalAndGrade({
                ascore: detail.ascore || 0,
                asScore: detail.asScore || 0,
                tscore: detail.tscore || 0,
                ftScore: detail.ftScore || 0
            });

            setSelectedAppeal(detail);
            setUpdatedScores({ ascore: detail.ascore || 0, asScore: detail.asScore || 0, tscore: detail.tscore || 0, ftScore: detail.ftScore || 0, totalScore, lectureGrade });
            setUpdatedAttendance({ newStatus: "" });
            setModalMode(mode);
            setShowModal(true);
        } catch (err) {
            console.error("이의제기 상세 조회 실패:", err);
        }
    };
    ...
    const getAttendanceTypeLabel = (status) => {
        if (!status) return "";
                ...
                <Modal.Body>
                    {selectedAppeal && (
                        <>
                            ...
                            {selectedAppeal.appealType === "ATTENDANCE" && (
                                <>
                                    <Form.Group className="mb-2">
                                        ...
                                        <Form.Control type="text" value={selectedAppeal.lectureDate || ""} disabled />
                                    </Form.Group>
                                    <Form.Group className="mb-2">
                            ...
                            {selectedAppeal.appealType !== "ATTENDANCE" && modalMode === "approve" && (
                                <>
                                    <Form.Group className="mb-2">

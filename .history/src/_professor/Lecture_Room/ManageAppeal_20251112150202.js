...
function ManageAppeal() {
    ...
    const WEIGHTS = { ascore: 20, asScore: 20, tscore: 30, ftScore: 30 }; // 점수명 반영
    ...
    const handleScoreChange = (e) => {
        ...
        // 빈 문자열이면 0으로 처리, 아니면 숫자 변환
        if (value === "" || /^\d+$/.test(value)) {
            const numValue = value === "" ? 0 : Number(value);  // parseInt 대신 Number 사용
                                ...
                                <>
                                    ...
                                    {["ascore", "asScore", "tscore", "ftScore"].map(k => (
                                        <Form.Group key={k} className="mb-2">
                                            ...
                                            <Form.Control type="number" name={k} value={updatedScores[k]} onChange={handleScoreChange} />
                                        </Form.Group>
                                    ))}
                                    <Form.Group className="mb-2">

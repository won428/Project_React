        ...
        <Container style={{ marginTop: 24 }}>
            ...
            {modalMode === "gradeView" && selectedAppeal && (
                <Modal show onHide={() => setModalMode("")} centered>
                    <Modal.Header closeButton><Modal.Title>성적 이의제기 상세보기</Modal.Title></Modal.Header>
                    <Modal.Body>
                        ...
                        {/* 첨부파일 이름 표시(상세보기용, disabled) */}
                        {selectedAppeal.attachments?.length > 0 && (
                            <Form.Group className="mb-3">
                                <Form.Label>첨부파일</Form.Label>
                                <ul style={{ paddingLeft: 0, listStyle: "none" }}>
                                    {selectedAppeal.attachments.map((file, idx) => (
                                    ...
                                    {selectedAppeal.attachments.map((file, idx) => (
                                        <li key={idx} style={{ marginBottom: "4px" }}>
                                            <Button
                                                ...
                                                onClick={() => downloadFile(file.storedKey, file.name)}
                                            >
                                                {file.name} 다운로드
                                            </Button>
                                        </li>
                ...
                <Modal show onHide={() => setModalMode("")} centered>
                    ...
                    <Modal.Body>
                        <Form.Group className="mb-2">
                            ...
                            <Form.Control type="text" value={selectedAppeal.attendanceDate ||

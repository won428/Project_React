...
function ManageAppeal() {
    ...
    // 📁 기존 downloadFile 함수 대신 아래 함수 사용
    const downloadFiles = async (files) => {
        if (!files || files.length === 0) return;

        try {
            for (const file of files) {
                const res = await axios.get(`${API_BASE_URL}/api/appeals/files/download/${file.storedKey}`, {
                    responseType: 'blob',
                });
                    ...
                    <Modal.Body>
                        ...
                        {selectedAppeal?.attachments?.length > 0 && (
                            <Form.Group className="mb-3">
                                ...
                                <Button size="sm" onClick={() => downloadFiles(selectedAppeal.attachments)}>
                                    전체 다운로드
                                </Button>
                            </Form.Group>
                        )}

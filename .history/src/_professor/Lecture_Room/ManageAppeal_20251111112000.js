...
import { useAuth } from "../../../public/context/UserContext"; // src 안으로 옮긴 가정
import { API_BASE_URL } from "../../config/config"; // src 안으로 옮긴 가정

function ManageAppeal() {
    const { user } = useAuth(); // 교수 로그인 정보
    const { lectureId } = useParams(); // 클릭한 강의 ID
    ...
    const [searchKeyword, setSearchKeyword] = useState(""); // 학생 이름 or 학번
    ...
    // 해당 강의 + 로그인한 교수가 담당인 경우만 이의제기 조회
    const fetchAppeals = () => {
        if (!lectureId || !user?.id) return;

        axios.get(`${API_BASE_URL}/api/appeals/lecture/${lectureId}`, {
            params: {
                receiverId: user.id // 교수 ID
            }
        })
    ...
    const approveAppeal = () => {
        ...
        axios.put(`${API_BASE_URL}/api/appeals/${selectedAppeal.appealId}/approve`, {
            ...updatedScores,
            sendingId: selectedAppeal.sendingId, // 학생 ID
            receiverId: user.id, // 교수 ID
            lectureId
        })
            .then(() => {

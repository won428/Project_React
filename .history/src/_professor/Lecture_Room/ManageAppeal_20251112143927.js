...
function App() {
    ...
    // ✅ 추가된 상태값 (출결 승인용)
    const [selectedAttendStatus, setSelectedAttendStatus] = useState("");
    ...
    const saveAppeal = () => {
        ...
        if (gradeTypes.includes(selectedAppeal.appealType)) {
            ...
            axios.put(`${API_BASE_URL}/api/appeals/${selectedAppeal.appealId}/updateStatus`, {
                ...
                receiverId: user.id,
                attendanceType: selectedAttendStatus, // ✅ 선택된 상태로 수정
                attendanceDetail: selectedReason       // ✅ 선택된 사유로 수정
            })
                .then(() => { setShowModal(false); fetchAppeals(); })
                .catch(err => console.error(err));
        }
    ...
    const handleApproveSubmit = async () => {
        ...
        try {
            await axios.put(`${API_BASE_URL}/api/appeals/${selectedAppeal.appealId}/updateStatus`, {
                attendanceType: selectedAttendStatus,
                attendanceDetail: selectedReason,
                receiverId: user.id
            });
            alert("출결 이의제기 처리가 완료되었습니다.");
            setShowModal(false);
            fetchAppeals();
        } catch (err) {
            console.error(err);
    ...
    return (
        <Container style={{ marginTop: 24 }}>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)} centered>
                        ...
                        <Form>
                            {["GRADE", "ASSIGNMENT", "MIDTERMEXAM", "FINALEXAM"].includes(selectedAppeal.appealType) ? (
                                ...
                                <>
                                    ...
                                    {/* ✅ 추가: 승인 시 선택하는 항목 */}
                                    <Form.Group className="mb-2">
                                        <Form.Label>출결 상태 변경</Form.Label>
                                        <Form.Select
                                            value={selectedAttendStatus}
                                            onChange={(e) => setSelectedAttendStatus(e.target.value)}
                                        >
                                            <option value="">출결 상태 선택</option>
                ...
                <Modal.Footer>
                    {selectedAppeal?.appealType === "ATTENDANCE" && (
                        <Button variant="success" onClick={handleApproveSubmit}>승인</Button>
                    )}

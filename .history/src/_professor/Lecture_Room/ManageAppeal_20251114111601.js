...
function ManageAppeal() {
    ...
    const downloadFile = async (storedKey, fileName) => {
        try {
            const res = await axios.get(`${API_BASE_URL}/api/appeals/files/download/${storedKey}`, {
            ...
            link.href = url;
            link.setAttribute('download', fileName || 'file'); // 파일명 지정
    ...
    const openModal = async (appeal, mode) => {
        if (appeal.appealType === "ATTENDANCE") {
            try {
                ...
                // attachments 포함
                setSelectedAppeal({ ...appeal, ...attendance, content: studentContent, attachments: appeal.attachments || [] });
            ...
            // attachments 포함
            setSelectedAppeal({ ...appeal, attachments: appeal.attachments || [] });

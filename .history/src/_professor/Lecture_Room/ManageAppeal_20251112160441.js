...
function ManageAppeal() {
    ...
    const openModal = (appeal, mode = "view") => {
        ...
        setModalMode(mode);
        setUpdatedAttendance({ newStatus: "" }); // 초기화
        setShowModal(true);
    };
    ...
    const getAttendanceTypeLabel = (attendStudent) => {
        if (!attendStudent) return "";
        // 배열이면 unique 처리 후, label 매핑
        const types = Array.isArray(attendStudent) ? [...new Set(attendStudent)] : [attendStudent];
        return types.map(t => ATTENDANCE_LABELS[t] || t).join(", ");
    };
                            ...
                            {modalMode === "approve" && selectedAppeal.appealType === "ATTENDANCE" && (
                                <>
                                    ...
                                    <Form.Group className="mb-2">
                                        ...
                                        <Form.Control type="text" value={selectedAppeal.appealDate} disabled />
                                    </Form.Group>
                                    <Form.Group className="mb-2">

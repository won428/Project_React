    ...
    useEffect(() => {
        ...
        axios.get(`${API_BASE_URL}/user/manageList`)
            .then(async (res) => {
                ...
                const applyRes = await axios.get(`${API_BASE_URL}/api/student/record/all`, {
                    params: { status: "PENDING" }
                });

                const pendingRecords = applyRes.data; // recordId, userId, status, ...
                        ...
                        students.map(student => (
                            <tr key={student.id}>
                                ...
                                <td>
                                    <Button
                                        variant="primary"
                                        onClick={() => navigate(`/user/StatusManage/${student.id}`)}
                                    >
                                        학적변경 처리
                                    </Button>
                                </td>

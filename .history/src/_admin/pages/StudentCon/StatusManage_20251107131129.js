...
function StatusManage() {
    ...
    // filteredGrades는 useMemo로 계산
    const filteredGrades = useMemo(() => {
        return grades.filter((g) => {
            if (!g.lecture?.lecStartDate) return false;
            const year = new Date(g.lecture.lecStartDate).getFullYear().toString().slice(2);
    ...
    // summary도 useMemo로 안전하게 계산
    const summary = useMemo(() => {
        if (filteredGrades.length === 0) return { majorAvg: 0, liberalAvg: 0, totalAvg: 0, semesterAvg: 0 };
        let majorSum = 0, liberalSum = 0, totalSum = 0;
        ...
        const semesterAvg = totalSum / filteredGrades.length;
        return {
    ...
    const updateStatus = () => alert(`학적 상태: ${selectedStatus}, 입학일: ${dates.admissionDate}`);
    const updateGradeScores = () => alert('성적 수정 완료 (더미)');
    const updateSummary = () => alert('학점 요약 수정 완료 (더미)');
    ...
    return (
        <Container style={{ marginTop: 24 }}>
            ...
            <Table bordered>
                <tbody>
                    <tr><td>전공 학점</td><td>{summary.majorAvg}</td></tr>
                    <tr><td>교양 학점</td><td>{summary.liberalAvg}</td></tr>
                    <tr><td>총 학점</td><td>{summary.totalAvg}</td></tr>
                    <tr><td>학기별 평균 학점</td><td>{summary.semesterAvg}</td></tr>
                </tbody>
            </Table>
            ...
            <Table bordered>
                <thead>
                    <tr><th>과목명</th><th>학점</th><th>점수</th></tr>
                </thead>
                <tbody>
                        ...
                        <tr key={idx}>
                            ...
                            <td>
                                <Form.Control
                                    ...
                                    onChange={(e) => {
                                        const copy = [...grades];
                                        const originalIndex = grades.findIndex(gr => gr === g);
                                        if (originalIndex > -1) copy[originalIndex].totalScore = e.target.value;
                                        setGrades(copy);
                                    }}
                                />
                            </td>
